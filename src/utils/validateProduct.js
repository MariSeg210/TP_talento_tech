export const validateProduct = (product) => {
  ...
  if (!product.file || product.file.length === 0) {
    ...
    if (product.file && product.file.type) {
      const urlPattern = ["image/jpeg", "image/png", "image/webp", "image/jpg"];
      if (!urlPattern.includes(product.file.type)) {
        errors.file =
          "La imagen del producto debe ser un archivo del tipo: png, jpg, jpeg o webp.";
      }
    }
